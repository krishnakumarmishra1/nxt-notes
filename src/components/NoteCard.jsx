...
export default function NoteCard({ note, fetchNotes, setEditingNote, allLabels, handleCopyNote, customNoteBgs, appTheme }) {
  ...
  // FIX: Text highlight only applied when background image is present
  const textHighlightClass = hasBgImage ? 'img-text-highlight' : '';

  if (note.is_trashed) {
    return (
      <div className={`border border-red-200 bg-red-50 p-5 relative flex flex-col justify-between min-h-[140px] opacity-80 ${shapeClass}`}>
        <div>
      ...
      <div className="flex flex-col gap-1.5 mb-2">
        {displayItems.map(item => (
          <div key={item.id} className="flex items-start gap-2">
            ...
            <span style={{ color: item.is_completed ? '#9ca3af' : textColor }} className={`text-sm leading-tight font-bold ${activeFont} ${textHighlightClass} ${item.is_completed ? 'line-through opacity-70' : ''}`}>{item.text}</span>
          </div>
        ))}
  ...
  return (
    <div 
      ...
      className={`relative group cursor-pointer transition-all duration-300 flex flex-col justify-between min-h-[140px] ${isMenuOpen ? 'z-[60] shadow-xl' : 'hover:z-50 hover:shadow-lg'}`}
    >
      <div 
        className={`absolute inset-0 z-0 transition-colors border border-gray-200 bg-cover bg-center ${shapeClass}`} 
        style={{ backgroundColor: note.color || '#ffffff', backgroundImage: note.bg_image_url ? `url(${note.bg_image_url})` : 'none' }} 
      />

      <div className="relative z-10 p-5 flex flex-col h-full justify-between">
        {isUploading && <div className="absolute inset-0 bg-white/60 backdrop-blur-sm z-20 flex items-center justify-center"><span className="text-sm font-semibold text-gray-800 bg-white px-4 py-1.5 rounded-full shadow-md">Uploading...</span></div>}

        {/* FIX: Pin Button gets a strong glass background if there is a BG Image */}
        ...
        <div className="flex-1">
          ...
          {note.is_checklist ? renderChecklist() : <p style={{ color: textColor }} className={`text-sm leading-relaxed whitespace-pre-wrap line-clamp-4 font-bold ${note.bg_image_url ? '' : 'mb-4'} ${activeFont} ${textHighlightClass}`}>{note.content}</p>}
