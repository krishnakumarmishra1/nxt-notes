...
export default function NoteToolbar({ 
  ...
  // FIX: अंदर (Edit Mode) और बाहर (Card Mode) दोनों जगह Glass Effect वापस ला दिया गया है!
  const toolbarContainerClass = isEditing 
    ? `flex items-center justify-between w-full mt-2 transition-opacity duration-200 relative z-[70] ${hasBgImage ? 'bg-white/85 backdrop-blur-2xl px-4 py-2.5 rounded-2xl shadow-lg border border-white/50' : 'pt-2 border-t border-black/10'}`
      ...
      <div className="flex items-center gap-1">
        ...
        {isEditing && (
          <div className="relative">
            ...
            {openMenu === 'font' && (
              <div className="absolute bottom-full left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-80 max-h-64 overflow-y-auto custom-scrollbar border border-white/40" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="relative">
          ...
          {openMenu === 'color' && (
            <div className="absolute bottom-full left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-64 border border-white/40 overflow-hidden" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}>
              <div className="absolute inset-0 bg-white/85 backdrop-blur-2xl z-0"></div>
              <div className="relative z-10 p-4 flex flex-col gap-3">
                <div>
                  <p className="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider mb-2">Colors</p>
                  <div className="flex flex-wrap gap-2">
                    {colors.map(color => <button key={color} type="button" onClick={() => { onThemeChange({ color: color === 'transparent' ? '' : color, bg_image_url: '' }); closeMenu(); }} className={`w-8 h-8 rounded-full border hover:scale-110 transition-transform flex items-center justify-center ${currentColor === color ? 'border-blue-500 shadow-md' : 'border-gray-300'}`} style={{ backgroundColor: color === 'transparent' ? '#fff' : color }}>{color === 'transparent' && <svg width="14" height="14" viewBox="0 0 24 24" stroke="gray" strokeWidth="2"><line x1="2" y1="2" x2="22" y2="22"/></svg>}</button>)}
                    <div className="relative w-8 h-8 rounded-full overflow-hidden border border-gray-300 shadow-sm flex items-center justify-center bg-white hover:bg-gray-100 transition-colors" title="Custom Note Color"><span className="text-gray-500 font-bold leading-none mb-0.5">+</span><input type="color" onChange={(e) => { onThemeChange({ color: e.target.value, bg_image_url: '' }); }} onBlur={closeMenu} className="absolute -top-2 -left-2 w-12 h-12 cursor-pointer opacity-0" /></div>
                  </div>
                </div>
                <div className="h-px bg-gray-300/50"></div>
                <div>
                  <p className="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider mb-2">Themes Gallery</p>
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    {defaultBgImages.map(bg => <button key={bg} type="button" onClick={() => { onThemeChange({ color: '', bg_image_url: bg }); closeMenu(); }} className={`w-8 h-8 rounded-full border hover:scale-110 transition-transform bg-cover bg-center ${currentBgImage === bg ? 'border-blue-500 shadow-md' : 'border-gray-300'}`} style={{ backgroundImage: `url(${bg})` }} title="Theme" />)}
                    {customNoteBgs.map(bg => <button key={bg} type="button" onClick={() => { onThemeChange({ color: '', bg_image_url: bg }); closeMenu(); }} className={`w-8 h-8 rounded-full border hover:scale-110 transition-transform bg-cover bg-center ${currentBgImage === bg ? 'border-blue-500 shadow-md' : 'border-gray-300'}`} style={{ backgroundImage: `url(${bg})` }} title="My Theme" />)}
                    {onCustomBgUpload && (
        ...
        <div className="relative"><button type="button" onClick={(e) => handleMenuClick(e, 'more')} className="text-gray-800 hover:text-blue-600 transition-colors p-2 rounded-full hover:bg-black/5" title="More"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" r="1"/><circle cx="12" cy="19" r="1"/></svg></button>
          ...
          {openMenu === 'more-shape' && <div className="absolute bottom-full left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-40 border border-white/40 overflow-hidden" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}><div className="absolute inset-0 bg-white/85 backdrop-blur-2xl z-0"></div><div className="relative z-10 py-2">{shapes.map(shape => <button key={shape.value} type="button" onClick={(e) => { e.stopPropagation(); onShapeChange(shape.value); closeMenu(); }} className={`w-full text-left px-5 py-2.5 text-sm font-bold hover:bg-white/60 transition-colors ${currentShape === shape.value ? 'text-blue-600' : 'text-gray-800'}`}>{shape.name}</button>)}</div></div>}
          {openMenu === 'more-label' && <div className="absolute bottom-full left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-60 border border-white/40 overflow-hidden" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}><div className="absolute inset-0 bg-white/85 backdrop-blur-2xl z-0"></div><div className="relative z-10 p-4"><input type="text" autoFocus placeholder="Enter label name" value={newLabelText} onChange={(e) => setNewLabelText(e.target.value)} className="w-full text-sm font-bold px-3 py-2 outline-none border border-gray-200 rounded-xl mb-3 text-gray-900 bg-white/50" /><div className="max-h-40 overflow-y-auto space-y-1 custom-scrollbar pr-1">{allLabels.map(label => <label key={label} className="flex items-center gap-3 w-full cursor-pointer hover:bg-white/60 p-2 rounded-xl"><input type="checkbox" checked={currentLabels.includes(label)} onChange={(e) => { e.stopPropagation(); onToggleLabel(label); }} className="w-4 h-4 text-blue-600 rounded" /><span className="text-sm font-bold text-gray-800">{label}</span></label>)}</div>{newLabelText.trim() !== '' && !allLabels.includes(newLabelText.trim()) && <button type="button" onClick={handleCreateLabel} className="mt-3 w-full text-left text-sm font-extrabold text-[var(--color-primary)] px-3 py-2 bg-blue-50/50 hover:bg-blue-100/50 rounded-xl transition-colors">+ Create "{newLabelText.trim()}"</button>}</div></div>}
        </div>
      </div>
