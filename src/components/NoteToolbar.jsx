...
export default function NoteToolbar({ 
  ...
  const colors = ['transparent', '#f28b82', '#fbbc04', '#fff475', '#ccff90', '#a7ffeb', '#cbf0f8', '#aecbfa', '#d7aefb'];
  ...
  const fonts = [
    { name: 'Default Sans', value: 'font-sans' }, { name: 'Roboto', value: 'font-roboto' }, { name: 'Open Sans', value: 'font-open-sans' }, { name: 'Montserrat', value: 'font-montserrat' }, { name: 'Poppins', value: 'font-poppins' }, { name: 'Raleway', value: 'font-raleway' }, { name: 'Nunito', value: 'font-nunito' }, { name: 'Oswald', value: 'font-oswald' }, { name: 'Anton (Bold)', value: 'font-anton' }, { name: 'Serif Elegant', value: 'font-serif-elegant' }, { name: 'Merriweather', value: 'font-merriweather' }, { name: 'Lora', value: 'font-lora' }, { name: 'Playfair', value: 'font-playfair' }, { name: 'Code / Mono', value: 'font-mono-code' }, { name: 'Fira Code', value: 'font-fira-code' }, { name: 'Inconsolata', value: 'font-inconsolata' }, { name: 'Handwriting', value: 'font-handwriting' }, { name: 'Comic', value: 'font-comic' }, { name: 'Dancing Script', value: 'font-dancing' }, { name: 'Indie Flower', value: 'font-indie' }, { name: 'Shadows Light', value: 'font-shadows' }, { name: 'Satisfy', value: 'font-satisfy' }
  ];
  ...
  // MOBILE: Flat transparent bar | DESKTOP: Floating Glass Pill
  const toolbarContainerClass = isEditing 
    ? "flex items-center justify-between w-full sm:max-w-[95%] mx-auto mt-2 sm:mt-4 px-2 sm:px-5 py-2 sm:py-2.5 sm:bg-white/95 sm:backdrop-blur-3xl sm:border border-gray-200 sm:shadow-2xl sm:rounded-full relative z-[70] transition-opacity duration-200"
      ...
      <div className="flex items-center gap-0 sm:gap-1">
        ...
        {isEditing && (
          <div className="relative">
            <button type="button" onClick={(e) => handleMenuClick(e, 'font')} className="text-gray-800 hover:text-blue-600 transition-colors p-2 sm:p-2 rounded-full hover:bg-black/5" title="Font Style"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="4 7 4 4 20 4 20 7"/><line x1="9" y1="20" x2="15" y2="20"/><line x1="12" y1="4" x2="12" y2="20"/></svg></button>
            {openMenu === 'font' && (
              <div className="absolute bottom-full left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-72 sm:w-80 max-h-64 overflow-y-auto custom-scrollbar border border-white/40" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}>
        ...
        {isEditing && (
          <div className="relative">
            <button type="button" onClick={(e) => handleMenuClick(e, 'textColor')} className="text-gray-800 hover:text-blue-600 transition-colors p-2 sm:p-2 rounded-full hover:bg-black/5" title="Text Color"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 20h16"/><path d="m6 16 6-12 6 12"/><path d="M8 12h8"/></svg></button>
            {openMenu === 'textColor' && (
              <div className="absolute bottom-full left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-48 border border-white/40 overflow-hidden" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}>
                <div className="absolute inset-0 bg-white/80 backdrop-blur-2xl z-0"></div>
              ...
              <div className="relative z-10 p-4 flex flex-col gap-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    {gradientColors.map(grad => <button key={grad} type="button" onClick={() => { onThemeChange({ color: grad, bg_image_url: '' }); closeMenu(); }} className={`w-8 h-8 rounded-full border hover:scale-110 transition-transform ${currentColor === grad ? 'border-blue-600 border-2 shadow-md' : 'border-white'}`} style={{ background: grad }} />)}
                    <div className="flex bg-white rounded-full p-0.5 gap-0 items-center border border-gray-300 shadow-sm hover:scale-105 transition-transform" title="Mix Custom Gradient">
                      <div className="relative w-7 h-7 rounded-full overflow-hidden shadow-sm" style={{ backgroundColor: grad1 }}><input type="color" value={grad1} onChange={(e) => { setGrad1(e.target.value); onThemeChange({ color: `linear-gradient(135deg, ${e.target.value} 0%, ${grad2} 100%)`, bg_image_url: '' }); }} className="absolute -top-2 -left-2 w-12 h-12 cursor-pointer opacity-0" /></div>
                      <div className="relative w-7 h-7 rounded-full overflow-hidden shadow-sm -ml-3 border-l-2 border-white" style={{ backgroundColor: grad2 }}><input type="color" value={grad2} onChange={(e) => { setGrad2(e.target.value); onThemeChange({ color: `linear-gradient(135deg, ${grad1} 0%, ${e.target.value} 100%)`, bg_image_url: '' }); }} className="absolute -top-2 -left-2 w-12 h-12 cursor-pointer opacity-0" /></div>
                    </div>
                    <div className="relative w-8 h-8 rounded-full overflow-hidden border border-gray-300 shadow-sm flex items-center justify-center bg-white hover:bg-gray-100 transition-colors" title="Custom Solid Color"><span className="text-gray-500 font-bold leading-none mb-0.5">+</span><input type="color" onChange={(e) => { onThemeChange({ color: e.target.value, bg_image_url: '' }); }} onBlur={closeMenu} className="absolute -top-2 -left-2 w-12 h-12 cursor-pointer opacity-0" /></div>
                  </div>
                </div>
        ...
        <div className="relative"><button type="button" onClick={(e) => handleMenuClick(e, 'more')} className="text-gray-800 hover:text-blue-600 transition-colors p-2 sm:p-2 rounded-full hover:bg-black/5" title="More"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" r="1"/><circle cx="12" cy="19" r="1"/></svg></button>
          ...
          {/* Sub-Menus for Card Hover View (Outside 3-Dots) */}
          {openMenu === 'more-font' && <div className="absolute bottom-full right-0 sm:left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-80 max-h-64 overflow-y-auto custom-scrollbar border border-white/60 bg-white/20 backdrop-blur-xl" onClick={(e) => e.stopPropagation()}><div className="relative z-10 p-2 grid grid-cols-2 gap-1.5">{fonts.map(font => <button key={font.value} type="button" onMouseEnter={() => onFontHover && onFontHover(font.value)} onMouseLeave={() => onFontHover && onFontHover(null)} onClick={(e) => { e.stopPropagation(); onFontChange(font.value); if(onFontHover) onFontHover(null); closeMenu(); }} className={`w-full text-left px-3 py-2 rounded-xl transition-all font-bold ${font.value} ${currentFont === font.value ? 'bg-blue-600 text-white shadow-md scale-105' : 'text-gray-900 bg-white/40 hover:bg-white/80 border border-white/50'}`}>{font.name}</button>)}</div></div>}
          {openMenu === 'more-textcolor' && <div className="absolute bottom-full right-0 sm:left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-48 border border-white/40 overflow-hidden" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}><div className="absolute inset-0 bg-white/80 backdrop-blur-2xl z-0"></div><div className="relative z-10 p-4 flex flex-wrap gap-2"><p className="text-[10px] font-extrabold text-gray-700 uppercase tracking-wider w-full mb-1">Text Color</p>{textColors.map(color => <button key={color} type="button" onClick={() => { onTextColorChange(color); closeMenu(); }} className={`w-8 h-8 rounded-full border hover:scale-110 transition-transform flex items-center justify-center ${currentTextColor === color ? 'border-blue-600 border-2 shadow-md' : 'border-gray-300'}`} style={{ backgroundColor: color }} />)}<div className="relative w-8 h-8 rounded-full overflow-hidden border border-gray-300 shadow-sm flex items-center justify-center bg-gray-50 hover:bg-gray-200 transition-colors" title="Custom Text Color"><span className="text-gray-500 font-bold leading-none mb-0.5">+</span><input type="color" onChange={(e) => onTextColorChange(e.target.value)} onBlur={closeMenu} className="absolute -top-2 -left-2 w-12 h-12 cursor-pointer opacity-0" /></div></div></div>}
          {openMenu === 'more-shape' && <div className="absolute bottom-full right-0 sm:left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-40 border border-white/40 overflow-hidden" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}><div className="absolute inset-0 bg-white/85 backdrop-blur-2xl z-0"></div><div className="relative z-10 py-2">{shapes.map(shape => <button key={shape.value} type="button" onClick={(e) => { e.stopPropagation(); onShapeChange(shape.value); closeMenu(); }} className={`w-full text-left px-5 py-2.5 text-sm font-bold hover:bg-white/60 transition-colors ${currentShape === shape.value ? 'text-blue-600' : 'text-gray-800'}`}>{shape.name}</button>)}</div></div>}
          {openMenu === 'more-label' && <div className="absolute bottom-full right-0 sm:left-0 mb-4 shadow-2xl rounded-3xl z-[90] w-60 border border-white/40 overflow-hidden" style={getMenuThemeStyle()} onClick={(e) => e.stopPropagation()}><div className="absolute inset-0 bg-white/85 backdrop-blur-2xl z-0"></div><div className="relative z-10 p-4"><input type="text" autoFocus placeholder="Enter label name" value={newLabelText} onChange={(e) => setNewLabelText(e.target.value)} className="w-full text-sm font-bold px-3 py-2 outline-none border border-gray-200 rounded-xl mb-3 text-gray-900 bg-white/50" /><div className="max-h-40 overflow-y-auto space-y-1 custom-scrollbar pr-1">{allLabels.map(label => <label key={label} className="flex items-center gap-3 w-full cursor-pointer hover:bg-white/60 p-2 rounded-xl"><input type="checkbox" checked={currentLabels.includes(label)} onChange={(e) => { e.stopPropagation(); onToggleLabel(label); }} className="w-4 h-4 text-blue-600 rounded" /><span className="text-sm font-bold text-gray-800">{label}</span></label>)}</div>{newLabelText.trim() !== '' && !allLabels.includes(newLabelText.trim()) && <button type="button" onClick={handleCreateLabel} className="mt-3 w-full text-left text-sm font-extrabold text-[var(--color-primary)] px-3 py-2 bg-blue-50/50 hover:bg-blue-100/50 rounded-xl transition-colors">+ Create "{newLabelText.trim()}"</button>}</div></div>}
        </div>

      </div>

      {/* Right Side: Undo/Redo & Close */}
      <div className="flex items-center gap-0 sm:gap-1 shrink-0 ml-auto">
        {isEditing && (
          <div className="flex items-center border-r border-gray-300/50 pr-1 sm:pr-2 mr-1 sm:mr-1">
            <button type="button" onClick={(e) => { e.stopPropagation(); onUndo(); }} disabled={!canUndo} className="text-gray-800 hover:text-blue-600 disabled:opacity-30 p-1.5 sm:p-2 rounded-full hover:bg-black/5" title="Undo"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 7v6h6"/><path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3l-3 2.7"/></svg></button>
            <button type="button" onClick={(e) => { e.stopPropagation(); onRedo(); }} disabled={!canRedo} className="text-gray-800 hover:text-blue-600 disabled:opacity-30 p-1.5 sm:p-2 rounded-full hover:bg-black/5" title="Redo"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 7v6h-6"/><path d="M3 17a9 9 0 0 1 9-9 9 9 0 0 1 6 2.3l3 2.7"/></svg></button>
          </div>
        )}
        
        {isEditing && <button type="button" onClick={(e) => { e.stopPropagation(); onClose(e); }} className="ml-1 sm:ml-3 px-4 sm:px-6 py-1.5 sm:py-2 text-sm font-extrabold text-white bg-gray-900 hover:bg-gray-800 rounded-full transition-colors shadow-md">Close</button>}
      </div>
