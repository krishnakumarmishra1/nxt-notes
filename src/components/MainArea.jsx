...
export default function MainArea({ notes, fetchNotes, selectedCategory, allLabels, searchQuery, viewMode, customNoteBgs, setCustomNoteBgs, appTheme }) {
  ...
  const handleCustomBgUpload = async (file, isEditingMode) => {
    ...
    try {
      ...
      if (isEditingMode) { handleEditChange('bg_image_url', publicUrl); handleEditChange('color', ''); } 
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showHistoryModal || isToolbarMenuOpen) return; 
  ...
  }, [isNewBoxExpanded, title, content, imageFile, newColor, newTextColor, newBgImage, newFont, newShape, newLabels, isNewChecklist, newChecklistItems, showHistoryModal, isToolbarMenuOpen]);
  ...
  const saveAndCloseEdit = async () => {
    if (!editingNote || showHistoryModal) return;
    setIsSaving(true);
    ...
    setEditingNote(null); fetchNotes(); setIsSaving(false);
  };
  ...
  // FIX: Text is Bold and Highlighted smoothly when there's an image
  const newTextHighlight = newBgImage ? 'img-text-highlight font-extrabold' : '';
  ...
  return (
    <main className="flex-1 flex flex-col bg-transparent h-full overflow-y-auto relative">
      ...
      {selectedCategory !== 'Trash' && selectedCategory !== 'Archive' && (
        <div className="p-8 pb-4">
          <div ref={newNoteRef} className={`max-w-3xl mx-auto relative group ${isToolbarMenuOpen ? 'z-[60]' : 'z-20'}`} onClick={() => setIsNewBoxExpanded(true)}>
            <div className={`absolute inset-0 z-0 border border-gray-300 shadow-md transition-all bg-cover bg-center ${newShape}`} style={{ backgroundColor: newColor || '#ffffff', backgroundImage: newBgImage ? `url(${newBgImage})` : 'none' }}>
            ...
            <div className="relative z-10 p-6 flex flex-col h-full">
              ...
              {isNewBoxExpanded && <input type="text" style={{ color: newTextColor }} placeholder="Title" value={title} onChange={(e) => handleNewChange('title', e.target.value)} className={`w-full outline-none text-2xl font-bold placeholder-gray-500 mb-3 bg-transparent px-2 ${activeFontNew} ${newTextHighlight}`} />}
              {isNewChecklist ? <ChecklistEditor items={newChecklistItems} setItems={setNewChecklistItems} fontClass={activeFontNew} textColor={newTextColor} highlightClass={newTextHighlight} /> : <textarea style={{ color: newTextColor }} placeholder="Write Your NXT Note..." value={content} onChange={(e) => handleNewChange('content', e.target.value)} className={`w-full outline-none text-lg placeholder-gray-600 resize-none bg-transparent font-medium px-2 ${activeFontNew} ${newTextHighlight} ${isNewBoxExpanded ? 'min-h-[100px] pb-10' : 'h-8'}`} />}
              {imagePreview && (
              ...
              {isNewBoxExpanded && (
                <div className="mt-4 pt-2 border-t border-black/10 flex items-center justify-between w-full">
                  <NoteToolbar 
                  ...
                  />
                  <button type="button" onClick={(e) => { e.stopPropagation(); setIsNewBoxExpanded(false); handleAddNote(); }} className="ml-3 px-6 py-2 text-sm font-extrabold text-white bg-gray-900 hover:bg-gray-800 rounded-full transition-colors shadow-md shrink-0">Close</button>
                </div>
              )}
            </div>
      ...
      <div className="p-8 pt-4">
        ...
        <div className={`max-w-7xl mx-auto ${viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5' : 'flex flex-col gap-4 max-w-3xl'}`}>
          {sortedNotes.map((note) => <NoteCard key={note.id} note={note} fetchNotes={fetchNotes} setEditingNote={openEditModal} allLabels={allLabels} handleCopyNote={handleCopyNote} customNoteBgs={customNoteBgs} appTheme={appTheme} />)}
        </div>
      </div>

      {/* Main Edit Modal */}
      {editingNote && !editingNote.is_trashed && (
        <div className={`fixed inset-0 bg-gray-900/60 flex items-center justify-center p-4 z-[60] backdrop-blur-md transition-opacity ${showHistoryModal ? 'opacity-0 pointer-events-none' : 'opacity-100'}`} onMouseDown={saveAndCloseEdit}>
          <div className="relative shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] z-50 rounded-2xl" onMouseDown={(e) => e.stopPropagation()}>
            
            <div className={`absolute inset-0 z-0 bg-cover bg-center ${editingNote.frame_style || 'shape-rounded'}`} style={{ backgroundColor: editingNote.color || '#ffffff', backgroundImage: editingNote.bg_image_url ? `url(${editingNote.bg_image_url})` : 'none' }}>
            ...
            <div className="relative z-10 p-6 flex flex-col h-full">
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                ...
                {(() => {
                  ...
                  const editTextHighlight = editingNote.bg_image_url ? 'img-text-highlight font-extrabold' : '';
                  return (
                    <>
                      <input type="text" style={{ color: editingNote.text_color || '#111827' }} value={editingNote.title} onChange={(e) => handleEditChange('title', e.target.value)} className={`w-full outline-none text-3xl font-bold mb-4 bg-transparent ${activeFontEdit} ${editTextHighlight}`} placeholder="Title" />
                      {editingNote.is_checklist ? <ChecklistEditor items={editingNote.checklist_items || []} setItems={(items) => setEditingNote({...editingNote, checklist_items: items})} fontClass={activeFontEdit} textColor={editingNote.text_color || '#111827'} highlightClass={editTextHighlight} /> : <textarea style={{ color: editingNote.text_color || '#111827' }} value={editingNote.content} onChange={(e) => handleEditChange('content', e.target.value)} className={`w-full outline-none text-xl resize-none flex-1 min-h-[200px] bg-transparent font-medium ${activeFontEdit} ${editTextHighlight}`} placeholder="Note content..." />}
                    </>
                  );
                })()}
              ...
              <div className="shrink-0 mt-4 pt-2 border-t border-black/10 flex items-center justify-between">
                <NoteToolbar 
                  isEditing={true} appTheme={appTheme}
                  ...
                  currentBgImage={editingNote.bg_image_url} onBgImageChange={(url) => handleEditChange('bg_image_url', url)} onCustomBgUpload={(file) => handleCustomBgUpload(file, true)} customNoteBgs={customNoteBgs}
                  ...
                  onDelete={async () => { await supabase.from('notes').update({ is_trashed: true }).eq('id', editingNote.id); setEditingNote(null); fetchNotes(); }} onCopy={(e) => { handleCopyNote(e, editingNote); setEditingNote(null); }} onArchive={async (e) => { e.stopPropagation(); await supabase.from('notes').update({ is_archived: !editingNote.is_archived }).eq('id', editingNote.id); setEditingNote(null); fetchNotes(); }} isArchived={editingNote.is_archived} onUndo={undoEdit} onRedo={redoEdit} canUndo={historyIndex > 0} canRedo={historyIndex < noteHistory.length - 1} onClose={saveAndCloseEdit}
                  hasBgImage={!!editingNote.bg_image_url}
                />
                
                {/* FIX: Time is beautifully placed on the right next to Close button */}
                <div className="flex items-center gap-3 ml-3 shrink-0">
                  {editingNote.updated_at && <span className="text-[11px] font-extrabold text-gray-500 whitespace-nowrap">Edited {formatTime(editingNote.updated_at || editingNote.created_at)}</span>}
                  <button type="button" onClick={saveAndCloseEdit} className="px-6 py-2 text-sm font-extrabold text-white bg-gray-900 hover:bg-gray-800 rounded-full transition-colors shadow-md">Close</button>
                </div>
      ...
      {showHistoryModal && editingNote && (
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center p-4 z-[90] backdrop-blur-md">
