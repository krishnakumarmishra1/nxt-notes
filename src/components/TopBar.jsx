...
export default function TopBar({ toggleSidebar, searchQuery, setSearchQuery, viewMode, setViewMode, openSettings }) {
  ...
  // यूज़र की डिटेल्स (Username) मंगाना
  useEffect(() => {
  ...
  return (
    <header className="h-16 bg-white/40 backdrop-blur-xl border-b border-white/50 flex items-center justify-between px-4 shrink-0 z-20 shadow-sm transition-colors">
      ...
      <div className="flex-1 max-w-2xl px-4">
        <div className="relative group">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="gray" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
          </div>
      ...
      <div className="flex items-center gap-2 relative">
        <button onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')} className="p-2 rounded-full hover:bg-black/10 text-gray-700 transition-colors hidden sm:block" title={viewMode === 'grid' ? "List View" : "Grid View"}>
          {viewMode === 'grid' ? <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg> : <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect width="7" height="7" x="3" y="3" rx="1"/><rect width="7" height="7" x="14" y="3" rx="1"/><rect width="7" height="7" x="14" y="14" rx="1"/><rect width="7" height="7" x="3" y="14" rx="1"/></svg>}
        </button>
        ...
        {isProfileOpen && (
          <div className="absolute top-12 right-0 mt-2 w-56 bg-white border border-gray-200 shadow-xl rounded-xl p-2 z-30">
            {/* User Info Display */}
            ...
            <button onClick={() => { setIsProfileOpen(false); openSettings(); }} className="w-full text-left px-4 py-2 text-sm text-gray-700 font-medium hover:bg-gray-50 rounded-lg transition-colors flex items-center gap-2 mb-1">
            ...
            <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-sm text-red-600 font-medium hover:bg-red-50 rounded-lg transition-colors flex items-center gap-2 border-t border-gray-100 pt-2">
